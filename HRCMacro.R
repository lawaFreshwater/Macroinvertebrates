...
fname <-"2018_csv_config_files/hrcMacro_config.csv"
...
for(i in 1:length(sites)){
  ...
  for(j in 1:length(Measurements)){
    ...
    url <- paste("http://tsdata.horizons.govt.nz/boo.hts?service=Hilltop",
                 "&request=GetData",
                 "&Site=",sites[i],
...
saveXML(con$value(), file=paste0("H:/ericg/16666LAWA/2018/MacroInvertebrates/1.Imported/",format(Sys.Date(),"%Y-%m-%d"),"/hrcMacro.xml"))
