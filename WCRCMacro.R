...
for(i in 1:length(sites)){
  for(j in 1:length(Measurements)){
    ...
    url <- paste("http://data.wcrc.govt.nz:9083/wq.hts?service=Hilltop",
                 "&request=GetData",
                 "&Site=",sites[i],
