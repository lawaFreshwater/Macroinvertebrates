...
fname <-"2018_csv_config_files/nccMacro_config.csv"
...
requestData <- function(url){
 ...
 ld <- function(url){
    (download.file(url,destfile="tmpncc",method="wininet",quiet=T))
    # pause(1)
    xmlfile <- xmlParse(file = "tmpncc")
    unlink("tmpr")
...
saveXML(con$value(), file=paste0("H:/ericg/16666LAWA/2018/MacroInvertebrates/1.Imported/",format(Sys.Date(),"%Y-%m-%d"),"/nccMacro.xml"))
