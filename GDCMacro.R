...
rm(list=ls())
loadLatestSiteTableMacro <- function(maxHistory=100){
  stepBack=0
  while(stepBack<maxHistory){
    if(dir.exists(paste0("h:/ericg/16666LAWA/2018/MacroInvertebrates/1.Imported/",format(Sys.Date()-stepBack,"%Y-%m-%d")))){
      if(file.exists(paste0("h:/ericg/16666LAWA/2018/MacroInvertebrates/1.Imported/",format(Sys.Date()-stepBack,"%Y-%m-%d"),"/LAWA_Site_Table_Macro.csv")))
        siteTable <- read.csv(paste0("h:/ericg/16666LAWA/2018/MacroInvertebrates/1.Imported/",format(Sys.Date()-stepBack,"%Y-%m-%d"),"/LAWA_Site_Table_Macro.csv"),stringsAsFactors=FALSE) #This is intersected with catchment info
      while(grepl(pattern = '^X',x = names(siteTable)[1])){
        siteTable=siteTable[,-1]
      }
      
      cat("loading macro siteTable  from",stepBack,'days ago\n')
      stepBack=1000
return(siteTable)
      }
    stepBack=stepBack+1
...
fname <-"H:/ericg/16666LAWA/2018/MacroInvertebrates/2018_csv_config_files/gdcMacro_config.csv"
...
ld <- function(url){
  (download.file(url,destfile="tmpgdc",method="wininet",quiet=T))
  # pause(1)
  xmlfile <- xmlParse(file = "tmpgdc")
  unlink("tmpr")
...
if(!dir.exists(paste0("H:/ericg/16666LAWA/2018/MacroInvertebrates/1.Imported/",format(Sys.Date(),"%Y-%m-%d")))){
  dir.create(paste0("H:/ericg/16666LAWA/2018/MacroInvertebrates/1.Imported/",format(Sys.Date(),"%Y-%m-%d")))
}
saveXML(con$value(), file=paste0("H:/ericg/16666LAWA/2018/MacroInvertebrates/1.Imported/",format(Sys.Date(),"%Y-%m-%d"),"/gdcMacro.xml"))
