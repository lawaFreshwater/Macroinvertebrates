...
fname <-"2018_csv_config_files/ecanMacro_config.csv"
...
sites = unique(siteTable$CouncilSiteID[siteTable$Agency%in%c('ECAN','Environment Canterbury','Christchurch')])
Measurements <- subset(df,df$Type=="Measurement")[,1]

#function to create xml file from url. 
ld <- function(url){
  str<- tempfile(pattern = "ecanfile", tmpdir = tempdir())
...
ld <- function(url){
  (download.file(url,destfile="tmpecan",method="wininet",quiet=T))
  # pause(1)
  xmlfile <- xmlParse(file = "tmpecan")
  unlink("tmpr")
...
for(i in i:length(sites)){
  ...
  for(j in 1:length(Measurements)){
    ...
    url <- paste("http://data.ecan.govt.nz/wqlawa.hts?service=Hilltop",
                 "&request=GetData",
                 "&Site=",sites[i],
    ...
    if(grepl(pattern = "%20&",x =  url,perl = TRUE)){
      url <- gsub(pattern = "%20&", replacement = "%20%26", x = url)
    }
    #------------------------------------------
...
saveXML(con$value(), file=paste0("H:/ericg/16666LAWA/2018/MacroInvertebrates/1.Imported/",format(Sys.Date(),"%Y-%m-%d"),"/ecanMacro.xml"))
