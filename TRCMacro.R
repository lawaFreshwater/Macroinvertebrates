...
fname <-"2018_csv_config_files/trcMacro_config.csv"
...
requestData <- function(url){
 ...
 ld <- function(url){
    (download.file(url,destfile="tmptrc",method="wininet",quiet=T))
    # pause(1)
    xmlfile <- xmlParse(file = "tmptrc")
    unlink("tmpr")
...
for(i in 1:length(sites)){
  ...
  for(j in 1:length(Measurements)){
    ...
    url <- paste("https://extranet.trc.govt.nz/getdata/LAWA_bio.hts?service=Hilltop",
                 "&request=GetData",
                 "&Site=",sites[i],
...
saveXML(con$value(), file=paste0("H:/ericg/16666LAWA/2018/MacroInvertebrates/1.Imported/",format(Sys.Date(),"%Y-%m-%d"),"/trcMacro.xml"))
