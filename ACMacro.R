...
# The process flag assumes this script will be called from a batch file. IF the flag is true, the code in this file will run.
  ...
  fname <-"H:/ericg/16666LAWA/2018/MacroInvertebrates/2018_csv_config_files/acMacro_config.csv"
  df <- read.csv(fname,sep=",",stringsAsFactors=FALSE)
  rm(fname)
  
  siteTable=read.csv(paste0('h:/ericg/16666LAWA/2018/MacroInvertebrates/1.Imported/LAWA_Site_Table_Macro.csv'),
                     stringsAsFactors = F)
  ...
  for(i in 1:length(sites)){
    ...
    for(j in 1:length(Measurements)){
      ...
      url <- paste("http://aklc.hydrotel.co.nz:8080/KiWIS/KiWIS?",
                   "datasource=2&service=SOS&version=2.0&",
                   "request=GetObservation&featureOfInterest=",sites[i], "&",
                   "observedProperty=", Measurements[j], "&",
                   "Procedure=raw&temporalfilter=om:phenomenonTime,P15Y", sep="")
      url <- gsub("%", "%25", url)
      url <- gsub(" ", "%20", url)
      xmlfile <- ld(url)
  ...
  saveXML(con$value(), file=paste0("H:/ericg/16666LAWA/2018/MacroInvertebrates/1.Imported/",format(Sys.Date(),"%Y-%m-%d"),"/acMacro.xml"))
